...
const MOD_DIR := "DarthAnonymous-FrameRateMeter" # Name of the directory that this file is in
const LOG_NAME := "DarthAnonymous-FrameRateMeter:Main" # Full ID of the mod (AuthorName-ModName)
...
func _ready() -> void:
	...
	var framerate_meter_script = load(mod_dir_path.path_join("framerate_meter.gd"))
	if framerate_meter_script:
		var framerate_meter = framerate_meter_script.new()
		add_child(framerate_meter)
		framerate_meter.show()
